...
double Trap(double left_endpt, double right_endpt, long int trap_count, 
   double base_len, int comm_sz, int my_rank, int n);    
...
int main(void) {
   ...
   */
   ...
   local_a = my_rank*h;
   local_b = b - (comm_sz-my_rank-1)*h;
   
   MPI_Barrier(MPI_COMM_WORLD);
   local_start = MPI_Wtime();
   local_int = Trap(local_a, local_b, local_n, h, comm_sz, my_rank, n);
...
double Trap(
   ...
   int first_i = my_rank + 1;
   int last_i = n - comm_sz - my_rank;
   
   for (i = first_i; i < last_i-1; i+=comm_sz) {
