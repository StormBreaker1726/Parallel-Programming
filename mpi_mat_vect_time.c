...
void Mat_vect_mult(
   ...
   for (local_i = 0; local_i < local_m; local_i++) {
      local_y[local_i] = 0.0;
      for (j = 0; j < n; j++)
         local_y[local_i] += local_A[local_i*n+j]*x[j] * sin(local_i)*cos(local_i)*sin(local_i)*cos(local_i)*tan(local_i) * sin(local_i)*cos(local_i)*sin(local_i)*cos(local_i)*tan(local_i);
   }
   free(x);
}  /* Mat_vect_mult */
